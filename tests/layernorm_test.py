...
if __name__ == "__main__":
    ...
    custom_ln = LayerNormFused(hidden_dim, eps=1e-6).cuda()
    
    torch_ln = nn.LayerNorm(hidden_dim, eps=1e-6).cuda().half()
    torch_ln.weight.data = custom_ln.weight.data.clone()
    torch_ln.bias.data.zero_() #my implamentation doesn't have bias
